...
if page == "Upload PropStream":
    ...
    up = st.file_uploader("Upload PropStream CSV", type="csv")
    if up:
...
# ─── Analytics Dashboard ───────────────────────────────────────────
    ...
    if "source" in df.columns:
        sel = st.multiselect("Filter by source", df["source"].unique(), default=None)
        if sel: df = df[df["source"].isin(sel)]
    if st.checkbox("Hot deals only"): df = df[df["hot_lead"]]
    mn,mx = df["date_posted"].dt.date.min(), df["date_posted"].dt.date.max()
    s,e   = st.slider("Date range", mn, mx, (mn,mx))
    df     = df[df["date_posted"].dt.date.between(s,e)]
    ...
    # chart w/ thicker grid
    df2 = df.dropna(subset=["price","date_posted"])
    chart = (
      alt.Chart(df2)
         .mark_line(point=True)
         .encode(
           x=alt.X("date_posted:T",title="Date"),
