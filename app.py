...
st.set_page_config(page_title="Savory Realty Investments",
                   page_icon="logo.png",
                   layout="wide")
...
<style>
  [data-testid="stAppViewContainer"] {{
    background: linear-gradient(rgba(0,0,0,0.6),rgba(0,0,0,0.6)),
                url("data:image/png;base64,{bg}") center/cover fixed no-repeat;
  }}
  .block-container {{ position: relative; z-index: 2; }}
  [data-testid="collapsedControl"] {{ position: fixed!important;top:0;left:0;
    z-index:99999;background:black!important;border-radius:0!important; }}
...
if page == "Upload PropStream":
    ...
    up = st.file_uploader("Upload a CSV file from PropStream", type="csv")
    if up:
...
# ─── Analytics Dashboard ─────────────────────────────────────────────────────
    ...
    if "source" in df.columns:
        ...
        sel = st.multiselect("Filter by source", src, default=src)
        if sel: df = df[df["source"].isin(sel)]
...
# ─── Settings (informational only) ─────────────────────────────────────────
    ...
    """)
    st.write("The Settings page is informational only—no runtime changes.")